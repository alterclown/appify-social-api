...
class IFeedService(ABC):
    @abstractmethod
    async def create_post(self, db: AsyncSession, author_id: UUID, text_content: str, image_url: Optional[str], privacy: str) -> JSONResponse: pass
...
class FeedService(IFeedService):
    ...
    async def create_post(self, db: AsyncSession, author_id: UUID, text_content: str, image_url: Optional[str], privacy: str) -> JSONResponse:
